...
// Service UUID
BLEService fallGuardService("19B10000-E8F2-537E-4F6C-D104768A1214");

// Characteristic: fall alert byte  (0 = no alert, 1 = alert active)
// Subscribed clients get a notification whenever the value changes.
BLEByteCharacteristic fallAlertCharacteristic(
  "19B10001-E8F2-537E-4F6C-D104768A1214",
  BLERead | BLENotify
);

// Characteristic: last predicted class index (0-6, 255 = no result yet)
BLEByteCharacteristic predictionCharacteristic(
  "19B10002-E8F2-537E-4F6C-D104768A1214",
  BLERead | BLENotify
);

// Characteristic: confidence scaled to 0-100 (uint8)
BLEByteCharacteristic confidenceCharacteristic(
  "19B10003-E8F2-537E-4F6C-D104768A1214",
  BLERead | BLENotify
);

// Characteristic: mode command written by the central (r/c/i as ASCII byte)
BLEByteCharacteristic modeCommandCharacteristic(
  "19B10004-E8F2-537E-4F6C-D104768A1214",
  BLERead | BLEWrite
);

// Track previous alert state so we only notify on transitions
bool previousFallAlertState = false;

// ─────────────────────────────────────────────────────────────
// Runtime constants & enums (unchanged from original)
// ─────────────────────────────────────────────────────────────
...
void printModeHelp() {
  ...
  Serial.println("BLE: write ASCII 'r'/'c'/'i' to modeCommand characteristic");
}

void printRuntimeStatus() {
  Serial.print("STATUS,mode=");            Serial.print(modeName(runtimeMode));
  Serial.print(",cal_ready=");             Serial.print(calibrationReady ? 1 : 0);
  Serial.print(",cal_count=");             Serial.print(calibrationWindowCount);
  Serial.print(",cal_target=");            Serial.print(kCalibrationWindowTarget);
  Serial.print(",window_fill=");           Serial.print(windowIndex);
  Serial.print("/");                        Serial.print(kWindowSamples);
  Serial.print(",fall_alerts=");           Serial.print(confirmedFallAlertCount);
  Serial.print(",stumble_predictions=");   Serial.println(stumblePredictionCount);
}

void announceMode(bool includeHint = true) {
  Serial.print("MODE,mode=");           Serial.print(modeName(runtimeMode));
  Serial.print(",sample_rate_hz=");     Serial.print(runtimeMode == MODE_RECORD ? 100 : kTrainingSampleRateHz);
  Serial.print(",window_samples=");     Serial.print(kWindowSamples);
  Serial.print(",cal_ready=");          Serial.println(calibrationReady ? 1 : 0);
...
void printCalibrationStatus() {
  Serial.print("CALIBRATION,status=");  Serial.print(calibrationReady ? "ready" : "collecting");
  Serial.print(",count=");              Serial.print(calibrationWindowCount);
  Serial.print(",target=");             Serial.print(kCalibrationWindowTarget);
  Serial.print(",mode=");               Serial.println(modeName(runtimeMode));
}
...
bool setupTinyMl() {
  ...
  if (inputFeatureCount != kFeatureCount) { Serial.println("ERROR,input_feature_count_mismatch");  return false; }
  if (outputClassCount  != kClassCount)   { Serial.println("ERROR,output_class_count_mismatch");   return false; }
  return true;
}
...
// ─────────────────────────────────────────────────────────────
// Fall decision (unchanged logic, BLE notification added below)
// ─────────────────────────────────────────────────────────────
...
/**
 * Push inference results and fall alert state to BLE characteristics.
 * Only writes when the value has actually changed to avoid flooding
 * the BLE stack with redundant notifications.
 */
void updateBleCharacteristics(
  const InferenceSummary& summary,
  const FallDecision&     decision
) {
  // Fall alert: notify only on rising/falling edge
  const uint8_t alertByte = decision.alertActive ? 1 : 0;
  if (decision.alertActive != previousFallAlertState) {
    fallAlertCharacteristic.writeValue(alertByte);
    previousFallAlertState = decision.alertActive;
  }

  // Prediction class index (0-6)
  predictionCharacteristic.writeValue((uint8_t)summary.bestIndex);

  // Confidence scaled to 0-100 for easy reading on the central side
...
// ─────────────────────────────────────────────────────────────
// Serial / BLE command handling
// ─────────────────────────────────────────────────────────────

/**
 * Apply a mode command character ('r', 'c', 'i', 'p', 'h').
 * Shared by both Serial and BLE command paths.
 */
...
/**
 * Check if the BLE central has written a new mode command.
 * The central writes one ASCII byte ('r', 'c', or 'i') to the
 * modeCommand characteristic.
 */
void handleBleCommands() {
  if (modeCommandCharacteristic.written()) {
    const uint8_t value = modeCommandCharacteristic.value();
...
bool setupBle() {
  if (!BLE.begin()) {
    Serial.println("ERROR,ble_init_failed");
    return false;
  }

  BLE.setLocalName("FallGuardNano33");
  BLE.setAdvertisedService(fallGuardService);

  fallGuardService.addCharacteristic(fallAlertCharacteristic);
  fallGuardService.addCharacteristic(predictionCharacteristic);
  fallGuardService.addCharacteristic(confidenceCharacteristic);
  fallGuardService.addCharacteristic(modeCommandCharacteristic);

  BLE.addService(fallGuardService);

  // Initial values
  fallAlertCharacteristic.writeValue(0);
  predictionCharacteristic.writeValue(255);   // 255 = no result yet
  confidenceCharacteristic.writeValue(0);
  modeCommandCharacteristic.writeValue(0);

  BLE.advertise();
  Serial.println("INFO,ble=advertising,name=FallGuardNano33");
  return true;
}

// ─────────────────────────────────────────────────────────────
// Inference printing (unchanged)
// ─────────────────────────────────────────────────────────────
...
void printInferenceSummary(
  ...
  Serial.print("INFER,mode=");              Serial.print(modeName(runtimeMode));
  Serial.print(",window=");                 Serial.print(inferenceWindowCounter);
  Serial.print(",pred=");                   Serial.print(labelForIndex(summary.bestIndex));
  Serial.print(",conf=");                   Serial.print(summary.bestScore, 4);
  Serial.print(",top2=");                   Serial.print(labelForIndex(summary.secondIndex));
  Serial.print(",top2_conf=");              Serial.print(summary.secondScore, 4);
  Serial.print(",fall_state=");             Serial.print(decision.state);
  Serial.print(",fall_candidate=");         Serial.print(decision.candidate ? 1 : 0);
  Serial.print(",fall_alert=");             Serial.print(decision.alertActive ? 1 : 0);
  Serial.print(",impact=");                 Serial.print(decision.impactDetected ? 1 : 0);
  Serial.print(",low_motion=");             Serial.print(decision.lowMotionDetected ? 1 : 0);
  Serial.print(",mag_mean=");               Serial.print(rawFeatureVector[kMagnitudeMeanFeatureIndex], 4);
  Serial.print(",mag_std=");                Serial.print(rawFeatureVector[kMagnitudeStdFeatureIndex], 4);
  Serial.print(",mag_max=");                Serial.print(rawFeatureVector[kMagnitudeMaxFeatureIndex], 4);
  Serial.print(",mag_range=");              Serial.print(rawFeatureVector[kMagnitudeRangeFeatureIndex], 4);
  Serial.print(",mag_energy=");             Serial.print(rawFeatureVector[kMagnitudeEnergyFeatureIndex], 4);
  Serial.print(",cal_ready=");              Serial.print(calibrationReady ? 1 : 0);
  Serial.print(",cal_count=");              Serial.print(calibrationWindowCount);
  Serial.print(",fall_alerts=");            Serial.print(confirmedFallAlertCount);
  Serial.print(",stumble_predictions=");    Serial.println(stumblePredictionCount);
...
void processWindowIfReady() {
  ...
  if (runtimeMode == MODE_CALIBRATE) {
    ...
    Serial.print("CALIBRATION,progress="); Serial.print(calibrationWindowCount);
    Serial.print(",target=");              Serial.print(kCalibrationWindowTarget);
    Serial.print(",ready=");               Serial.println(calibrationReady ? 1 : 0);
    if (calibrationReady) {
...
void setup() {
  ...
  // BLE init — non-fatal: device still works over Serial if BLE fails
  if (!setupBle()) {
    Serial.println("WARN,ble_unavailable_continuing_without_ble");
  }
...
void loop() {
  ...
  // BLE.poll() handles connection events and incoming writes
  // without blocking the sensor sampling loop.
  BLE.poll();

  // LED mirrors BLE connection state
  ...
  // ── Sensor sampling (timing-gated, unchanged) ────────────
